...
6. **Individual `@massdriver/ui` imports** (`import Box from '@massdriver/ui/Box'`). No `@mui/material` imports anywhere except `theme/MassdriverThemeScope.tsx`. Monochrome UI-chrome icons render bare; colored/brand icons use `IconTile`.
7. **MUI v5 renders only inside `MassdriverThemeScope`.** Never use MUI `Drawer`/`Modal`/`Slide`-based components — they don't paint reliably in Backstage's hybrid MUI v4 + v5 runtime. Overlays are absolutely-positioned `Box` panels (see `InstanceDrawer`, `DeploymentLogsPanel`).
